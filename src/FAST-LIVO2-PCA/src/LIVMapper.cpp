      ...
      extR(M3D::Identity())
{
  ...
  root_dir = ROOT_DIR;  // 在CmakeLists.txt里面自动获取并定义的
  initializeFiles();
...
void LIVMapper::readParameters(ros::NodeHandle &nh)
{
  ...
  nh.param<double>("preprocess/filter_size_surf", filter_size_surf_min, 0.5); // 默认0.5m，实际设置为了0.1m
...
{
  downSizeFilterSurf.setLeafSize(filter_size_surf_min, filter_size_surf_min, filter_size_surf_min); // 设置体素降采样立方体的大小为0.1m x 0.1m x 0.1m
  extT << VEC_FROM_ARRAY(extrinT);
  extR << MAT_FROM_ARRAY(extrinR);
  //extR_和extT_在fast-livo2中被认为是常量，与fast-lio2不同,不作为状态变量
...
void LIVMapper::initializeSubscribersAndPublishers(ros::NodeHandle &nh, image_transport::ImageTransport &it) 
{
  ...
  sub_imu = nh.subscribe(imu_topic, 200000, &LIVMapper::imu_cbk, this); // 当回调线程被阻塞时数据会被积压到最大200000条直到阻塞被释放
  sub_img = nh.subscribe(img_topic, 200000, &LIVMapper::img_cbk, this);
  // 关于回调函数操作参考[数据采集缓冲区](./Supplementary/callback数据采集缓冲区.png)
...
{    
  ...
  downSizeFilterSurf.setInputCloud(feats_undistort); // 体素的分割是以(x_min, y_min, z_min)作为起点按步长向外扩张的
  downSizeFilterSurf.filter(*feats_down_body); // 滤波结果是每个体素内所有点的质心
  
  double t_down = omp_get_wtime();

  feats_down_size = feats_down_body->points.size();
  voxelmap_manager->feats_down_body_ = feats_down_body;
  transformLidar(_state.rot_end, _state.pos_end, feats_down_body, feats_down_world); // 将点云的位置和强度转换到世界坐标系下
  voxelmap_manager->feats_down_world_ = feats_down_world;
  voxelmap_manager->feats_down_size_ = feats_down_size;
  

  // 从这里开始进行 voxel_map_ 的构建，PCA项目对此处开始所用到的函数进行替换
  if (!lidar_map_inited) // 下采样点云作为观测数据会在初始化时仅被使用一次，后续更新都在 StateEstimation 函数中被使用
  {
    lidar_map_inited = true;
    voxelmap_manager->BuildVoxelMap();
  }
  // 到这里相当于完成了体素地图的构建， voxel_map_ 以哈希表形式组织着很多 VoxelOctoTree 根节点，每个根节点下面可能存在着8个VoxelOctoTree子节点
  // PCA项目对 VoxelOctoTree 进行替换，设定每个根节点可能存在2个子节点

  double t1 = omp_get_wtime();

  voxelmap_manager->StateEstimation(state_propagat); // 基于IMU传播的先验状态进行ESIKF，输入是先验状态和测量点云，输出是最优估计状态
  _state = voxelmap_manager->state_;
  _pv_list = voxelmap_manager->pv_list_; // 获取最后一次迭代初始状态下的世界系降采样点云（包括未匹配到平面的点）

  double t2 = omp_get_wtime();

  if (imu_prop_enable) // 是否以IMU频率发布估计状态，一般情况下是关闭的，因为IMU频率较高，而LIO频率较低
  {
    ekf_finish_once = true;
    latest_ekf_state = _state;
    latest_ekf_time = LidarMeasures.last_lio_update_time;
  ...
  transformLidar(_state.rot_end, _state.pos_end, feats_down_body, world_lidar); // 由于最后一次迭代的最优状态还未作用于 pv_list_ ，这里额外变换一次
  for (size_t i = 0; i < world_lidar->points.size(); i++) 
  {
    ...
    var = (_state.rot_end * extR) * var * (_state.rot_end * extR).transpose() +
          (-point_crossmat) * _state.cov.block<3, 3>(0, 0) * (-point_crossmat).transpose() + _state.cov.block<3, 3>(3, 3); // 根据后验状态更新点云的方差
    ///同理这里的协方差更新与论文不一致，参考 [issue#89]() ////////////
  ...
  if(voxelmap_manager->config_setting_.map_sliding_en) // 当探测范围过大时，可以启用滑动窗口策略，将窗口外的体素删除。默认不启用相当于认为内存够大直接建全局地图
  {
    voxelmap_manager->mapSliding();
  }
  
  PointCloudXYZI::Ptr laserCloudFullRes(dense_map_en ? feats_undistort : feats_down_body); // 使用稠密点云还是体素降采样点云，默认使用稠密点云
  int size = laserCloudFullRes->points.size();
  PointCloudXYZI::Ptr laserCloudWorld(new PointCloudXYZI(size, 1));

  for (int i = 0; i < size; i++) 
  {
    RGBpointBodyToWorld(&laserCloudFullRes->points[i], &laserCloudWorld->points[i]); // 实际并没有RGB，只是转换到世界系同时赋值强度
  }
  *pcl_w_wait_pub = *laserCloudWorld;

  publish_frame_world(pubLaserCloudFullRes, vio_manager); // 将 pcl_w_wait_pub 上的点有选择地发布
  if (pub_effect_point_en) publish_effect_world(pubLaserCloudEffect, voxelmap_manager->ptpl_list_); //是否发布成功匹配到平面特征的点
  if (voxelmap_manager->config_setting_.is_pub_plane_map_) voxelmap_manager->pubVoxelMap();         //是否发布以面特征组成的体素地图，不同颜色表示面的不确定性
  publish_path(pubPath);                  // 发布轨迹
  publish_mavros(mavros_pose_publisher);  // 发布mavros格式的估计状态
  ...
  // euler_cur = RotMtoEuler(_state.rot_end); // 这里同样是作者残留的多余计算，可以注释掉
  // 将每帧的必要信息写入文件 mat_out.txt 中
  fout_out << std::setw(20) << LidarMeasures.last_lio_update_time - _first_lidar_time << " " << euler_cur.transpose() * 57.3 << " "
            << _state.pos_end.transpose() << " " << _state.vel_end.transpose() << " " << _state.bias_g.transpose() << " "
            << _state.bias_a.transpose() << " " << V3D(_state.inv_expo_time, 0, 0).transpose() << " " << feats_undistort->points.size() << std::endl;
}
...
void LIVMapper::run() 
{
  ...
  while (ros::ok()) 
  {
    ...
    stateEstimationAndMapping(); // 内部依次执行LIO或VIO
  }
  savePCD();
}
...
bool LIVMapper::sync_packages(LidarMeasureGroup &meas)
{
  ...
  case ONLY_LIO: 
  {
    ...
    mtx_buffer.lock(); // 欲将共享内存 imu_buffer 中晚于last_lio_update_time且早于img_capture_time的IMU数据取到m.imu中，需先上锁防止其他线程操作该内存
    while (!imu_buffer.empty())
    {
      if (imu_buffer.front()->header.stamp.toSec() > meas.lidar_frame_end_time) break;
      m.imu.push_back(imu_buffer.front()); //从头取
      imu_buffer.pop_front(); //取完即删
    }
    lid_raw_data_buffer.pop_front();
    lid_header_time_buffer.pop_front();
    mtx_buffer.unlock();
    sig_buffer.notify_all(); //解锁并通知被阻塞的线程
  ...
  case LIVO:
  {
    ...
    EKF_STATE last_lio_vio_flg = meas.lio_vio_flg; // 构造函数初始化是0.WAIT
    // double t0 = omp_get_wtime();
    switch (last_lio_vio_flg)
    {
    // double img_capture_time = meas.lidar_frame_beg_time + exposure_time_init;
    case WAIT:
    case VIO: // 为下一步将做的LIO进行准备lidar数据和imu数据
    {
      ...
      while (!imu_buffer.empty())
      {
        ...
        if (imu_buffer.front()->header.stamp.toSec() > meas.last_lio_update_time) m.imu.push_back(imu_buffer.front()); //从头取
      ...
      // 前面限制了last_lio_update_time < img_capture_time < lid_newest_time ,所以对于lid_raw_data_buffer内一帧完整的点云需要切分成前后两部分
      // 上一次切剩的后半部分是pcl_proc_next作为这一次的pcl_proc_cur，这一次切的后半部分作为pcl_proc_next供下一次使用
      *(meas.pcl_proc_cur) = *(meas.pcl_proc_next);
      ...
      while (!lid_raw_data_buffer.empty())
      {
        ...
        lid_raw_data_buffer.pop_front(); // 用完即删
        lid_header_time_buffer.pop_front();
      }

      meas.measures.push_back(m);
      meas.lio_vio_flg = LIO;  // 已为LIO准备好所需的数据
...
void LIVMapper::publish_frame_world(const ros::Publisher &pubLaserCloudFullRes, VIOManagerPtr vio_manager)
{
  ...
  laserCloudmsg.header.frame_id = "camera_init"; // 世界系
  ...
  if (pcd_save_en) // 是否将点云使用 pcd_writer.writeBinary 保存到 all_points_dir 文件中
  {
...
void LIVMapper::publish_odometry(const ros::Publisher &pubOdomAftMapped)
{
  ...
  br.sendTransform( tf::StampedTransform(transform, odomAftMapped.header.stamp, "camera_init", "aft_mapped") ); // camera_init是世界坐标系的原点
