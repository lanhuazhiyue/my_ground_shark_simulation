...
struct MeasureGroup
{
  double vio_time;
  double lio_time;
  deque<sensor_msgs::Imu::ConstPtr> imu; // deque是双端队列，支持在头部和尾部进行插入和删除操作
  cv::Mat img;
  MeasureGroup()
  {
    vio_time = 0.0;
...
typedef struct pointWithVar
{
  ...
  Eigen::Matrix3d body_var; //体坐标系（雷达系）下的协方差矩阵
  Eigen::Matrix3d var;      //世界系下的协方差矩阵
  Eigen::Matrix3d point_crossmat; //雷达坐标系下的点的反对称矩阵
