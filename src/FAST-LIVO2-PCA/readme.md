...
# 1.3. PCA 改进详解
学术上有个领域“[点云分割](https://blog.csdn.net/luolaihua2018/article/details/120113848)”就是专门研究这个事的，最经典的是方法是RANSAC（Random Sample Consensus）。
## 1.3.1. 动机
...
《切向的选取规则》：切分目的是尽可能让子体素更大概率拟合出完整平面，最小特征值足够小表示平面可信，所以对于一团点云，应该让最小特征值方向上的点云变得更加集中，于是切向自然而然地应选取为**切Z轴**，这样切向更可能平行于潜在平面，避免割裂已有完整平面。

#### 1.3.2.1.2. 切点的选取
在Z轴上的主成分坐标系原点是Z轴方向上所有点云的质心，如果存在离群点会严重影响质心的位置，切Z轴的目的是使Z轴上的点云分布更加集中，方差更小，又最小类内方差等价于最大化类间方差，也就是说可以使用OTSU 算法（大津法）确定切点。

《切点的选取规则》：

### 1.3.2.2. 合并策略
超体（super voxel） 聚类，区域生长算法
